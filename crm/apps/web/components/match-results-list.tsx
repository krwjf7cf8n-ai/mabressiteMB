...
import { ShareMatchResults } from "./share-match-results";
...
export function MatchResultsList({
  ...
  emptyContext,
  contextLabel = "Resultados de compatibilidade",
}: {
  items: MatchListItem[];
  totalEvaluated: number;
  eliminationReasonTally: Array<{ reason: string; count: number }>;
  emptyContext: string;
  /** G31 — rótulo usado no resumo de "Compartilhar" (ex.: "Imóveis compatíveis com Fulano"). */
  contextLabel?: string;
}) {
  if (totalEvaluated === 0) {
  ...
  return (
    <div className="space-y-3">
      <ShareMatchResults contextLabel={contextLabel} items={items} />
